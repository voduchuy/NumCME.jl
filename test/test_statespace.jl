using EtFsp.statespace: FspStateSpaceBasic, rstep_expand!, get_state_count
using Test
...
# Test constructors
@test stateset = FspStateSpaceBasic(bursting_gene.stoich_mat, bursting_gene.x0) isa Any 
@test stateset = FspStateSpaceBasic(bursting_gene.stoich_mat, [bursting_gene.x0]) isa Any

# Test that state exploration routines return state spaces with correct sizes
bursting_statespace = FspStateSpaceBasic(bursting_gene.stoich_mat, bursting_gene.x0)
@test get_state_count(bursting_statespace) == 1
@test rstep_expand!(bursting_statespace, 0) isa Any 
@test get_state_count(bursting_statespace) == 1

rstep_expand!(bursting_statespace, 1)
@test get_state_count(bursting_statespace) == 3

bursting_statespace = FspStateSpaceBasic(bursting_gene.stoich_mat, bursting_gene.x0)
rstep_expand!(bursting_statespace, 3)
@test get_state_count(bursting_statespace) == 7

bursting_statespace = FspStateSpaceBasic(bursting_gene.stoich_mat, bursting_gene.x0)
rstep_expand!(bursting_statespace, 4)
...
# Test constructors
@test stateset = FspStateSpaceBasic(toggle_switch.stoich_mat, toggle_switch.x0) isa Any 
@test stateset = FspStateSpaceBasic(toggle_switch.stoich_mat, [toggle_switch.x0]) isa Any

# Test that state exploration routines return state spaces with correct sizes
toggle_statespace = FspStateSpaceBasic(toggle_switch.stoich_mat, toggle_switch.x0)
@test get_state_count(toggle_statespace) == 1
@test rstep_expand!(toggle_statespace, 0) isa Any 
@test get_state_count(toggle_statespace) == 1

rstep_expand!(toggle_statespace, 1)
@test get_state_count(toggle_statespace) == 3

toggle_statespace = FspStateSpaceBasic(toggle_switch.stoich_mat, toggle_switch.x0)
rstep_expand!(toggle_statespace, 3)
